...
let numeroMasChico = Number(numerosDeLaLista[0].innerText);
let numeroMasGrande = Number(numerosDeLaLista[0].innerText);
let numeroMasRepetido;
let mayorFrecuencia = 0;
let cantidadDeNumeros = 0;

for (let i = 0; i < numerosDeLaLista.length; i++) {
    let numero = Number(numerosDeLaLista[i].innerText);
    ...
    for (let j = 0; j < numerosDeLaLista.length; j++) {
        if (Number(numerosDeLaLista[j].innerText) === numero) {
